...
class DC_SUP:
    ...
    def __init__(self, dc: str):
        ...
        # self.__get_json_file(f"{self.path}/data/dict-1.0.0.json")
        self.dict: dict[str, dict] = self.dc_sup['dict']
        ...
        self.cgi: dict[str, dict[str, dict[str, dict[str, dict[str, dict[str, dict[str,
                                                                                   dict[str, dict[str, dict[str, dict[str, dict[str, dict]]]]]]]]]]]] = self.data['cgi']
        self.encam: dict[str, dict[str, dict[str, dict[str,
                                                       dict[str, dict]]]]] = self.dc_sup['forwarding']
            ...
            for cod_cnl, tipo_orig_data in sup_cnl_data.items():
                ...
                if not data.get(reg):
                    data[reg] = {
                        ...
                        # SE 190: Municipal(CEO,CPG,CRC,CVY,CCP,PGC)
                        'descr': {},
                    }
        ...
        for reg in data:
            ...
            if self.capa.get(reg):
                ...
                for tipoOrig, to_data in self.capa[reg].items():
                    if len(to_data['sx_a']):
                        sx_a.append(
                            f"{tipoOrig}: ({",".join(to_data['sx_a'].values())})")
                    if len(to_data['sx_b']):
                        sx_b.append(
                            f"{tipoOrig}: ({",".join(to_data['sx_b'].values())})")
                    if len(to_data['tg_a']):
                        tg_a.append(
                            f"{tipoOrig}: ({",".join(to_data['tg_a'].values())})")
                    if len(to_data['tg_b']):
                        tg_b.append(
                            f"{tipoOrig}: ({",".join(to_data['tg_b'].values())})")
                sx_a = '\n'.join(sx_a)
                sx_b = '\n'.join(sx_b)
                tg_a = '\n'.join(tg_a)
                tg_b = '\n'.join(tg_b)
    ...
    def __data_extract_encam(self):
                ...
                for rn1, rota_summary_data in rn1_data.items():
                    ...
                    for rota_summary, formato_summary_data in rota_summary_data.items():
                        ...
                        for formato_summary, traducao_data in formato_summary_data.items():
                            self.__level_open(
                                f'Formato Summary: {formato_summary}')
                            ...
                            for idSup_Cod_CNL_a_idTipoOrig_idEncam, erro_ord_encam in sup_encam.items():
                                idSup, cod_cnl_a, idTipoOrig, idEncam = str(
                                    idSup_Cod_CNL_a_idTipoOrig_idEncam).split(',')
                                tipoOrig_data: dict = self.dict['tipo_orig'][idTipoOrig]
                                tipoOrig = tipoOrig_data['TipoOrig']
                                self.__level_open(
                                    f'idSup: {idSup}, Cod_CNL_a: {cod_cnl_a}, TipoOrig: {idTipoOrig}-{tipoOrig}')
                                ...
                                idTarifacao = str(
                                    sup_data[tipoOrig_data['idTarifacao']])
                                ...
                                self.__level_item(
                                    f'RN1_a........: {rn1_a}-{olo}')
                                self.__level_item(
                                    f'Abrangencia..: {idAbrangencia}-{abrangencia}')
                                self.__level_item(
                                    f'TipoSrv......: {idTipoSrv}-{tipo_serv}')
                                self.__level_item(
                                    f'Tarifacao....: {idTarifacao}-{tarifacao}')
                                self.__level_item(
                                    f'TrType.......: {TrType}-{tipo_tr}')
                                ...
                                self.__level_item(
                                    f'CNL_a........: {sigla_cnl_a}-{municipio_a}')
                                ...
                                reg_a = al_data_a['Reg']
                                self.__level_item(
                                    f'AL_a.........: {idAL_a}-{al_a}/{uf_a}-{reg_a} ({cn_a}) ROP: {rop_a}')

                                cn_b, idAL_b, idPortado, grp, traducao = sup_data[
                                    'cnl'][cod_cnl_a][idTipoOrig]
                                AbrCN, _idTipoOrig, idTipoDest, idACB, rn1_b = str(
                                    grp).split(',')
                                idAL_b = str(idAL_b)
                                ...
                                reg_b = al_data_b['Reg']
                                self.__level_item(
                                    f'AL_b.........: {idAL_b}-{al_b}/{uf_b}-{reg_b} ({cn_b}) ROP: {rop_b}')
        ...
        for reg in data:
            ...
            data[reg] = data[reg].sort_values(
                by=['Origem', 'Central Destino'], ascending=[True, True])
    ...
    def __summary_trunks(self, sx_a, sx_b, ord_encam, idEncam, tipoOrig, rotas, reg):
        ...
        for id_encam_type, rotas_data_det in rotas_data.items():
            self.__level_open(
                f'encam.sx.id_encam_type: {idEncam}.{sx_a}.{id_encam_type}')
            ...
            if arr_rota:
                ...
                encam_type: str = arr_encam_type['Quando']+(
                    ' Transbordo' if arr_encam_type['Transbordo'] else '')
    ...
    def __group_encam_data(self, df: pd.DataFrame):
        ...
        def aplicar_distinct(series):
            ...
            valores_unicos = [
                v for v in valores_unicos if str(v).strip() != '']
    ...
    def __group_cgi_data(self, df: pd.DataFrame):
        ...
        grouped['sort_key'] = grouped['G'].astype(
            str) + grouped['Sigla_CNL'].astype(str)
    ...
    def __excel_format_capa(self, title: str, ws: Worksheet):
        ...
        fill_Y1 = PatternFill(start_color="FFCC00",
                              end_color="FFCC00", fill_type="solid")
        fill_Y2 = PatternFill(start_color="FFFF99",
        ...
        for row in ws.iter_rows():
            ...
            for cell in row:
                if cell.value and cell.alignment and cell.alignment.wrap_text:
                    ...
                    largura_coluna = ws.column_dimensions[get_column_letter(
                        cell.column)].width
                    linhas_texto = len(texto) / \
                        (largura_coluna * 0.7)  # aproximação
    ...
    def __excel_worksheet_auto_width(self, ws: Worksheet):
        ...
        for column in ws.columns:
            ...
            for cell in column:
                try:
                    max_length = max(
                        max_length, self.__get_cell_width(str(cell.value)))
                except:
                    pass

            ws.column_dimensions[column_letter].width = min(
                max_length + 2, self.excel_max_width)
    ...
    def __get_json_file(self, file) -> dict:
        try:
            ...
            print(
                "Erro ao decodificar JSON. Verifique se o arquivo está no formato correto.")
        except Exception as e:
    ...
    def __level_item(self, text, c=''):
        if self.verbose < 2 or self.level < 0:
            return
        if self.verbose == 2:
